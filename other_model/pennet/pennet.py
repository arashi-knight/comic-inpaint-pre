...
from other_model.pennet.core.spectral_norm import use_spectral_norm
...
class InpaintGenerator(BaseNetwork):
    def __init__(self, config, init_weights=True):  # 1046
        super(InpaintGenerator, self).__init__()
        cnum = 32

        if config.is_gray:
            self.dw_conv01 = nn.Sequential(
                nn.Conv2d(2, cnum, kernel_size=3, stride=2, padding=1),
        ...
        if config.is_gray:
            ...
            self.torgb5 = nn.Sequential(
                nn.Conv2d(cnum * 32, 1, kernel_size=1, stride=1, padding=0),
                nn.Tanh())
            self.torgb4 = nn.Sequential(
                nn.Conv2d(cnum * 16, 1, kernel_size=1, stride=1, padding=0),
                nn.Tanh())
            self.torgb3 = nn.Sequential(
                nn.Conv2d(cnum * 8, 1, kernel_size=1, stride=1, padding=0),
                nn.Tanh())
            self.torgb2 = nn.Sequential(
                nn.Conv2d(cnum * 4, 1, kernel_size=1, stride=1, padding=0),
                nn.Tanh())
            self.torgb1 = nn.Sequential(
                nn.Conv2d(cnum * 2, 1, kernel_size=1, stride=1, padding=0),
                nn.Tanh())
            self.decoder = nn.Sequential(
                nn.Conv2d(cnum * 2, cnum, kernel_size=3, stride=1, padding=1),
                nn.ReLU(),
                nn.Conv2d(cnum, 1, kernel_size=3, stride=1, padding=1),
                nn.Tanh()
            )
        else:
            # torgb
...
if __name__ == '__main__':
    ...
    print('Number of parameter of generator: {}'.format(total_params / 1e6))
    total_params = sum(p.numel() for p in discriminator.parameters() if p.requires_grad)
    print('Number of parameter of core: {}'.format(total_params / 1e6))
    import sys
    ...
    pyramid_imgs, output = generator(img, mask)
    print('pyramid_imgs: ', len(pyramid_imgs))
