...
import config
# from svae.utils import imsave


class ScreenVAE(nn.Module):
    def __init__(self,inc=1,outc=4, outplanes=64, downs=5, blocks=2,load_ext=True, 
        save_dir=config.Config.svae_model_path, init_type="normal", init_gain=0.02, gpu_ids=[]):
...
if __name__ == "__main__":
    ...
    # line = torch.randn(1, 1, 256, 256)

    # 读取图片
    img = Image.open('../examples/test/imgs/image.jpg').convert('L')
    line = Image.open('../examples/test/lines/000085.png').convert('L')
    ...
    # print_parameters(svae)
    # print_parameters(svae.dec)
    # print_model_layers(svae,x = img, line = line)
